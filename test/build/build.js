...
const pathSeparator = /[\\/]/;
const contractPath_default = "./contracts/PaypolitanToken.sol";
...
function build(fullContractPath, theLogger) {
    ...
    if (!fullContractPath) fullContractPath = contractPath_default;

    let index = fullContractPath.lastIndexOf("/");
    if (index === -1) index = fullContractPath.lastIndexOf("\\");
    if (index === -1) {
...
function compileContract(contractPath, contractFile) {
    if (log)
        logger.log(
            "==> Compiling contract '" + contractPath + contractFile + "' and dependencies..."
        );
    ...
    const abi = compiledObject.contracts["PaypolitanToken.sol"]["PaypolitanToken"].abi;
    this.abi = abi.sort((a, b) => {
        if (lower(a.name) < lower(b.name)) return -1;
        if (lower(a.name) > lower(b.name)) return 1;
        return 0;
    });
...
function saveOutputFile(filename, data) {
    ...
    fs.writeFileSync(filename, data, function(err, data) {
        if (err)
            if (log)
                logger.log(colors.red.bold("!!! Error while saving '" + filename + "': " + err));
    ...
    if (!files.has(contractFile)) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.startsWith("import ")) {
                const dependencyFullPath = parseAndResolveImportPath(
                    contractBase,
                    substituteDependencyLine(line)
                );
...
function parseAndResolveImportPath(contractBase, line) {
    ...
    if (baseParts.length > 0 && baseParts[baseParts.length - 1] === "") baseParts.pop(); // Remove trailing '/' from path component
    ...
    for (let i = 0; i < baseParts.length; i++) {
        ...
        if (i !== baseParts.length - 1) resolvedPath += "/";
    }
...
function replaceAll(value, oldSeparator, newSeparator) {
    ...
    for (let i = 0; i < parts.length; i++) {
        result += parts[i];
        if (i !== parts.length - 1) result += newSeparator;
    }
    return result;
}
